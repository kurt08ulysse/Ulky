...
<head>
    <meta charset="UTF-8">
    <title>Quittance de Démarche - {{ $receipt->receipt_number }}</title>
    <style>
...
<body>
    <div class="header">
        ...
        <div style="margin-top: 8px; font-weight: 600; color: #4B5563;">ADMINISTRATION MUNICIPALE — DÉMARCHES ADMINISTRATIVES</div>
    </div>

    <div class="receipt-title">
        Quittance de Frais de Démarche<br>
    ...
    <table class="meta-table">
        <tr><td class="meta-label">Date d'émission :</td><td>{{ now()->format('d/m/Y H:i') }}</td></tr>
        <tr><td class="meta-label">Demandeur :</td><td><strong>{{ $user->name }}</strong><br>Tél : {{ $user->phone ?? 'Non renseigné' }}</td></tr>
        <tr><td class="meta-label">Référence démarche :</td><td style="font-family: monospace;">{{ $request->reference }}</td></tr>
        <tr><td class="meta-label">Type :</td><td>{{ $request->type }}</td></tr>
    </table>

    <div style="clear: both; height: 10px;"></div>

    <table class="details-table">
        <thead><tr><th>Désignation</th><th style="text-align: right; width: 180px;">Montant</th></tr></thead>
        <tbody>
            <tr>
                <td><strong>{{ $request->title }}</strong><br><span style="font-size: 12px; color: #6B7280;">Démarche #{{ $request->id }}</span></td>
                <td style="text-align: right;">{{ number_format($request->fee_amount / 100, 0, ',', ' ') }} FCFA</td>
            </tr>
            <tr class="total-row">
    ...
    <table class="meta-table" style="background-color: #F9FAFB; padding: 15px; border: 1px solid #E5E7EB;">
        <tr><td class="meta-label" style="width: 180px;">Méthode de paiement :</td><td>{{ $payment->operator === 'airtel_money' ? 'Airtel Money' : 'Moov Money' }}</td></tr>
        <tr><td class="meta-label">Numéro de transaction :</td><td style="font-family: monospace; font-weight: bold;">{{ $payment->transaction_id ?? 'N/A' }}</td></tr>
        <tr><td class="meta-label">Date du règlement :</td><td>{{ $request->paid_at ? \Carbon\Carbon::parse($request->paid_at)->format('d/m/Y H:i:s') : now()->format('d/m/Y H:i:s') }}</td></tr>
    </table>
